...
- `Schema.toArbitrary`;
- `Schema.Arbitrary`;
- `Schema.Annotations.ToArbitrary` and the `toArbitrary` annotation;
...
The new `toCodecArbitrary` annotation describes a generatable representation as a Schema `Link`:
...
If the previous annotation supplied a recognized constructive constraint, move it under
`toCodecArbitrary.constraint` and adapt its shape. The predicate remains authoritative:
